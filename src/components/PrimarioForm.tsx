...
export function PrimarioForm({ item, onSave, onCancel }: {
  item?: ProdutoPrimario;
  onSave?: (item: ProdutoPrimario) => void;
  onCancel?: () => void;
}) {
  const [_, setPrimario] = useAtom(primarioAtom);
  ...
  function handleSubmit(e: React.FormEvent) {
    ...
    if (onSave) {
      ...
      setPrimario((prev: ProdutoPrimario[]) => {
  ...
  return (
    <form onSubmit={handleSubmit} className="bg-white rounded shadow p-4 w-full max-w-lg mx-auto">
      <h2 className="text-lg font-bold mb-4">{item ? 'Editar' : 'Adicionar'} Item Primário</h2>
