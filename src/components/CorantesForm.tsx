...
export function CorantesForm({ item, onSave, onCancel }: {
  item?: ProdutoPrimario;
  onSave?: (item: ProdutoPrimario) => void;
  onCancel?: () => void;
}) {
  const [_, setCorantes] = useAtom(corantesAtom);
  ...
  function handleSubmit(e: React.FormEvent) {
    ...
    if (onSave) {
      ...
      setCorantes((prev: ProdutoPrimario[]) => {
  ...
  return (
    <form onSubmit={handleSubmit} className="bg-white rounded shadow p-4 w-full max-w-lg mx-auto">
      <h2 className="text-lg font-bold mb-4">{item ? 'Editar' : 'Adicionar'} Corante</h2>
