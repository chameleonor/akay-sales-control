...
export function EssenciasForm({ item, onSave, onCancel }: {
  item?: ProdutoPrimario;
  onSave?: (item: ProdutoPrimario) => void;
  onCancel?: () => void;
}) {
  const [_, setEssencias] = useAtom(essenciasAtom);
  ...
  function handleSubmit(e: React.FormEvent) {
    ...
    if (onSave) {
      ...
      setEssencias((prev: ProdutoPrimario[]) => {
  ...
  return (
    <form onSubmit={handleSubmit} className="bg-white rounded shadow p-4 w-full max-w-lg mx-auto">
      <h2 className="text-lg font-bold mb-4">{item ? 'Editar' : 'Adicionar'} Essência</h2>
